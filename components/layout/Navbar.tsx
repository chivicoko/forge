...
import { ShoppingBag, Menu, X, Sun, Moon, ChevronDown, LayoutDashboard, LogOut, User } from "lucide-react"
...
export function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            {/* Logo */}
            <Link href="/" className="flex items-center gap-2 font-black text-xl tracking-tight shrink-0" aria-label="FORGE Home">
            ...
            <div className="flex items-center gap-1.5">
              ...
              {rates && rates.length > 0 && (
                <Select value={effectiveRateId} onValueChange={setRateId}>
                  ...
                  <SelectContent>
                    {rates.map((r) => r.id && (
                      <SelectItem key={r.id} value={r.id} className="text-xs">{r.currency} — {r.country}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              )}
              ...
              {isSignedIn ? (
                  ...
                  {userMenuOpen && (
                    <>
                      ...
                      <div className="absolute right-0 mt-2 w-52 bg-background border rounded-xl shadow-xl z-20 py-1.5 overflow-hidden">
                        ...
                        <Link href="/dashboard" className="flex items-center gap-2 px-3 py-2 text-sm hover:bg-accent transition-colors" onClick={() => setUserMenuOpen(false)}>
                          <LayoutDashboard className="h-4 w-4 text-amber-500" /> Dashboard
                        </Link>
                        <Link href="/order-tracking" className="flex items-center gap-2 px-3 py-2 text-sm hover:bg-accent transition-colors" onClick={() => setUserMenuOpen(false)}>
                ...
                <SignInButton mode="modal">
                  <Button variant="outline" size="sm" className="hidden md:flex h-8 text-xs font-semibold">Sign In</Button>
                </SignInButton>
              )}
              ...
              {/* Mobile hamburger */}
              <Button variant="ghost" size="icon" className="md:hidden h-8 w-8" onClick={() => setMobileOpen(true)} aria-label="Open menu">
      ...
      <div className={`fixed inset-0 z-50 bg-background transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "translate-x-full"} md:hidden flex flex-col`}>
        <div className="flex items-center justify-between p-4 border-b">
          <Link href="/" className="font-black text-xl flex items-center gap-2" onClick={() => setMobileOpen(false)}>
        ...
        <nav className="flex flex-col gap-1 p-4 flex-1">
          ...
          {isSignedIn && (
            <>
              <Link href="/dashboard" className="px-4 py-3 text-base font-semibold rounded-xl hover:bg-accent transition-colors flex items-center gap-2" onClick={() => setMobileOpen(false)}>
                <LayoutDashboard className="h-4 w-4 text-amber-500" /> Dashboard
              </Link>
              <Link href="/order-tracking" className="px-4 py-3 text-base font-semibold rounded-xl hover:bg-accent transition-colors flex items-center gap-2" onClick={() => setMobileOpen(false)}>
                <ShoppingBag className="h-4 w-4 text-amber-500" /> My Orders
              </Link>
            </>
          )}
        ...
        <div className="p-4 border-t">
          {isSignedIn ? (
            <Button variant="outline" className="w-full gap-2 text-destructive border-destructive" onClick={() => { signOut(); setMobileOpen(false) }}>
              <LogOut className="h-4 w-4" /> Sign Out
            </Button>
          ) : (
            <SignInButton mode="modal">
              <Button className="w-full bg-amber-500 hover:bg-amber-600 text-white">Sign In</Button>
            </SignInButton>
          )}
        </div>
      </div>
