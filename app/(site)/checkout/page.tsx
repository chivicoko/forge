...
import { getRateByIdQueryOptions, listRatesQueryOptions, makeOrder } from "@/lib/api"
...
import { ShieldCheck, Loader2, X, SquareMinus, SquarePlus, ArrowLeft } from "lucide-react"
...
const checkoutSchema = z.object({
  ...
  payment_gateway: z.enum(["stripe"], { required_error: "Select a payment method" }),
})
type CheckoutData = z.infer<typeof checkoutSchema>

export default function CheckoutPage() {
  const { cart, removeFromCart, increaseQuantity, decreaseQuantity, cartTotalPrice, cartCount, clearCart } = useCart()
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      else { toast.success("Order placed!"); clearCart(); router.push("/order-tracking") }
    },
    onError: (err: Error) => toast.error(err.message || "Order failed. Please try again."),
  })
  ...
  const onSubmit = (values: CheckoutData) => {
    if (!isSignedIn) { toast.error("Please sign in to checkout"); return }
    if (cartCount() < 1) { toast.warning("Cart is empty"); return }

    mutation.mutate({
      item: cart.map((i) => ({
        product: i.id, name: i.name, price: i.price / rate,
        quantity: i.quantity, color: i.colors ?? [], size: i.sizes ?? [],
        image: i.images?.[0],
      })),
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {!isSignedIn ? (
          <div className="max-w-sm mx-auto text-center py-16 space-y-4">
            ...
            <p className="text-muted-foreground text-sm">You need an account to complete your purchase.</p>
            <SignInButton mode="modal">
              <Button className="w-full bg-amber-500 hover:bg-amber-600 text-white">Sign In to Checkout</Button>
            </SignInButton>
          </div>
        ) : (
            ...
            <div className="lg:col-span-3 space-y-5">
              ...
              <div className="bg-background border rounded-2xl p-6">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {[
                    { name: "phone_number" as const, label: "Phone Number*", placeholder: "+234 903 473 9483" },
                    { name: "streetNo" as const, label: "Street Address*", placeholder: "4 Olukoya Azikiwe St." },
                    ...
                    { name: "zip" as const, label: "ZIP / Postal Code", placeholder: "100012" },
                  ].map(({ name, label, placeholder }) => (
                    <div key={name}>
                      <label className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-1.5 block">{label}</label>
                      <Input {...register(name)} placeholder={placeholder} className="h-10" />
              ...
              <div className="bg-background border rounded-2xl p-6">
                ...
                {cart.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <Button asChild variant="link" className="text-amber-500"><Link href="/store">Browse store</Link></Button>
                  </div>
                ) : (
                    ...
                    {cart.map((item) => (
                      <div key={item.id} className="flex gap-3 pb-4 border-b last:border-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-amber-500 font-bold text-sm">{symbol}{(item.price / rate).toFixed(2)}</p>
                          {item.sizes?.length > 0 && <p className="text-xs text-muted-foreground">Size: {item.sizes.join(", ")}</p>}
                          {item.colors?.length > 0 && <p className="text-xs text-muted-foreground">Color: {item.colors.join(", ")}</p>}
                          <div className="flex items-center gap-2 mt-2">
                            <button type="button" onClick={() => decreaseQuantity(item.id)} aria-label="Decrease quantity"><SquareMinus className="h-4 w-4 hover:text-amber-500 transition-colors" /></button>
                            <span className="text-sm font-semibold w-5 text-center">{item.quantity}</span>
                            <button type="button" onClick={() => increaseQuantity(item.id)} aria-label="Increase quantity"><SquarePlus className="h-4 w-4 hover:text-amber-500 transition-colors" /></button>
                          </div>
                        </div>
                        <button type="button" onClick={() => removeFromCart(item.id)} className="self-start text-muted-foreground hover:text-destructive transition-colors" aria-label={`Remove ${item.name}`}>
                          <X className="h-4 w-4" />
                        </button>
                      </div>
                    ))}
              ...
              <div className="bg-background border rounded-2xl p-6 sticky top-20 space-y-5">
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between"><span className="text-muted-foreground">Subtotal</span><span>{symbol}{(cartTotalPrice() / rate).toFixed(2)}</span></div>
                    ...
                    <div className="flex justify-between font-black text-lg">
                      <span>Total</span>
                      <span className="text-amber-500">{symbol}{(cartTotalPrice() / rate).toFixed(2)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <label className="flex items-center gap-3 p-3 border-2 border-amber-500 rounded-xl bg-amber-500/5 cursor-pointer">
                    <input type="radio" {...register("payment_gateway")} value="stripe" className="accent-amber-500 w-4 h-4" defaultChecked />
                  ...
                  {errors.payment_gateway && <p className="text-xs text-destructive mt-1">{errors.payment_gateway.message}</p>}
                </div>
